...
- 为扫描队列、URL 去重记录、结果历史、单次扫描规则数、响应正文、HTTP 响应等待和正则匹配设置资源上限，避免 Burp 长时间运行时资源无限增长。
...
- 移除未使用的 IntelliJ `forms_rt` 依赖及其传递依赖，以及无调用的 `CustomHelpers`、旧自定义请求头辅助方法和孤儿文案资源。
- 新增 GitHub Actions Maven 构建，并扩充规则完整性、持久化、资源边界、正则超时和扫描计数回归测试。
