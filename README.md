...
1. 在 Burp 中加载插件。
2. 插件首次启动后会把 JAR 内置规则写入当前运行目录的 `Rules.yaml`；已有规则文件不会被覆盖。
3. 在配置页开启你需要的开关，例如被动扫描、携带请求头。
4. 正常测试目标站点，插件会自动递归检查各层路径。
5. 在结果页查看命中记录，并联动查看请求和响应。
6. 如需针对某个站点补扫，可右键请求发送到 RouteVulScan。

为避免长时间运行拖垮 Burp，插件最多保留 5,000 条结果和 50,000 条 URL 去重记录，待扫描队列与单次规则数上限均为 1,000。单请求响应超时为 30 秒；超过 5 MiB 的正文不执行正则匹配，单次正则匹配超过 2 秒会被终止。所有超限情况都会写入 Burp Errors 面板。
